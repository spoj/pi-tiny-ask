...
The tool can also generate an image with pi's configured Google, Google Vertex, OpenAI, or OpenRouter authentication. Set `output` to the workspace-relative path where the image should be saved. Examples are `google/gemini-3.1-flash-image`, `google-vertex/gemini-3.1-flash-image`, `openai/gpt-image-2`, and `openrouter/bytedance-seed/seedream-4.5`. All four accept optional reference images through `files`. With `output`, both OpenAI `api` values select the Images API: generation without files, edits with reference images. OpenRouter uses its `/images` endpoint only when no explicit `api` override is supplied. That endpoint supports routing `only`, `order`, `ignore`, `sort`, `allow_fallbacks`, and provider-specific `options`; other configured routing options, including `data_collection` and `zdr`, are rejected rather than dropped. Text sampling parameters are not sent to image endpoints. Vertex generation supports a configured API key or ADC with project and location.
